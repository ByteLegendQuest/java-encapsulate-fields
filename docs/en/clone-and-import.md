...
`git clone https://github.com/ByteLegendQuest/java-encapsulate-fields`

A project directory named `java-encapsulate-fields` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-encapsulate-fields` directory which is created just now.
